...
{
	// We have modified this function to remove the existent limit on worker threads available for Linux
	// and to exposed the possibility to change this number when launching the simulator.

	const int32 NbPhysicalCores = FPlatformMisc::NumberOfCores();
	const int32 NbLogicalCores = FPlatformMisc::NumberOfCoresIncludingHyperthreads();

	UE_LOG(LogInit, Log, TEXT("NumberOfWorkerThreadsToSpawn:"));
	UE_LOG(LogInit, Log, TEXT(" - Number of physical cores available for the process: %d"), NbPhysicalCores);
	UE_LOG(LogInit, Log, TEXT(" - Number of logical cores available for the process: %d"), NbLogicalCores);

	int32 NumberWorkers;
	const bool cl_number_workers = FParse::Value(FCommandLine::Get(), TEXT("workersthreadpool"), NumberWorkers);
	if (cl_number_workers) {
		// By engine limitations, we restrict to 26 the maximum possible threads to spawn
		NumberWorkers = FMath::Min(NumberWorkers, 26);

		/*
		// This is a debug output to check thread pool workers when compiled in release
		ANSICHAR Msg[110];
		memset(Msg, 0, sizeof(Msg));
		sprintf(Msg, "NumberOfWorkerThreadsToSpawn::FromCmdLine: NbPhysicalCores: %d    -    NumberOfWorkerThreadsToSpawn: %d\n", NbPhysicalCores, NumberWorkers);
		write(STDOUT_FILENO, Msg, sizeof(Msg));
		*/

		UE_LOG(LogInit, Log, TEXT(" - Worker number selected throught command line: %d"), NumberWorkers);

		return NumberWorkers;
	}

	// By default, the worker number is going to be one less than the physical cores avaiable, two as a minimum or 26 as maximum.
	NumberWorkers = FMath::Min(FMath::Max(NbPhysicalCores - 1, 2), 26);
	UE_LOG(LogInit, Log, TEXT(" - Worker number by default: %d (you can change this number with the command line parameter '-workersthreadpool X', for a max of 26 threads)"), NumberWorkers);

	/*
	// This is a debug output to check thread pool workers when compiled in release
	ANSICHAR Msg[110];
	memset(Msg, 0, sizeof(Msg));
	sprintf(Msg, "NumberOfWorkerThreadsToSpawn::Default: NbPhysicalCores: %d    -    NumberOfWorkerThreadsToSpawn: %d\n", NbPhysicalCores, NumberWorkers);
	write(STDOUT_FILENO, Msg, sizeof(Msg));
	*/
