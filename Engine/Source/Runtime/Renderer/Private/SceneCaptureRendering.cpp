...
static void UpdateSceneCaptureContentDeferred_RenderThread(
	...
	FGBufferRequest* InOutGBufferRequest = nullptr)
{
	...
	FDeferredUpdateResource::UpdateResources(RHICmdList);
	{
		...
		// Render the scene normally
		{
			...
			SceneRenderer->Render_CARLA(RHICmdList, InOutGBufferRequest);
		}
		...
		FRDGBuilder GraphBuilder(RHICmdList);
		{
			...
			if (InOutGBufferRequest != nullptr && InOutGBufferRequest->IsRequested(EGBufferTextureID::SceneColor))
			{
				auto Readback = InOutGBufferRequest->Readbacks[0].Get();
				AddReadbackTexturePass(GraphBuilder, RDG_EVENT_NAME("EXTRACT SCENE COLOR"), OutputTexture, [Readback, OutputTexture](FRHICommandList& RHICmdList)
				{
					Readback->EnqueueCopyRDG(RHICmdList, OutputTexture->GetRHI());
				});
			}
		}
...
{
	UpdateSceneCaptureContentsWithGBuffers(CaptureComponent, nullptr);
}

void FScene::UpdateSceneCaptureContentsWithGBuffers(class USceneCaptureComponent2D* CaptureComponent, FGBufferRequest* InOutGBufferRequest)
{
	...
	if (UTextureRenderTarget2D* TextureRenderTarget = CaptureComponent->TextureTarget)
	{
		...
		ENQUEUE_RENDER_COMMAND(CaptureCommand)(
			[SceneRenderer, TextureRenderTargetResource, EventName, bGenerateMips, GenerateMipsParams, bDisableFlipCopyGLES, InOutGBufferRequest](FRHICommandListImmediate& RHICmdList)
			{
	...
	for (uint32 CaptureIter = StartIndex; CaptureIter < EndIndex; ++CaptureIter)
	{
		...
		if (GetFeatureLevel() >= ERHIFeatureLevel::ES3_1 && TextureTarget)
		{
			...
			for (int32 faceidx = 0; faceidx < (int32)ECubeFace::CubeFace_MAX; faceidx++)
			{
				...
				{
					UpdateSceneCaptureContent_RenderThread(RHICmdList, SceneRenderer, TextureRenderTarget, TextureRenderTarget, EventName, FResolveParams(FResolveRect(), TargetFace), false, FGenerateMipsParams(), false, nullptr);
				}
				);
			}
		}
