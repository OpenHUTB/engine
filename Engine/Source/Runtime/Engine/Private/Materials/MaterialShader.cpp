...
void FMaterialShaderMap::GetAllOutdatedTypes(TArray<const FShaderType*>& OutdatedShaderTypes, TArray<const FShaderPipelineType*>& OutdatedShaderPipelineTypes, TArray<const FVertexFactoryType*>& OutdatedFactoryTypes)
{
	...
	// 用于材质烘焙出错的调试：打印当前执行 所有 材质着色器映射(AllMaterialShaderMaps) 的索引值，当在函数出错时，将会打印出错的索引
	for (int i = 0; i < AllMaterialShaderMaps.Num(); i++)
	{
		const FMaterialShaderMap* ShaderMap = AllMaterialShaderMaps[i];
		UE_LOG(LogMaterial, Display, TEXT("GetAllOutdatedTypes->AllMaterialShaderMaps Index = %d"), i);
	...
	bIsPersistent(true)
{
	...
	CompileTime = 0.f;
	{
		...
		// 用于材质烘焙出错的调试：打印AllMaterialShaderMaps的个数，
		// 可以通过这个个数判断打印的母材质实例名字这条信息对应的AllMaterialShaderMaps中的索引值
#if USE_CUSTOM_ENGINE
		UE_LOG(LogMaterial, Display, TEXT("FMaterialShaderMap->AllMaterialShaderMaps Num: %d"), AllMaterialShaderMaps.Num());
#endif
	}
#endif
}
