...
static inline void ConvertRawD32S8DataToFLinearColor(uint32 Width, uint32 Height, uint8* In, uint32 SrcPitch, FLinearColor* Out, FReadSurfaceDataFlags InFlags)
{
	bool bLinearToGamma = InFlags.GetLinearToGamma();
	// Depth
	if (!InFlags.GetOutputStencil())
	{
		for (uint32 Y = 0; Y < Height; Y++)
		{
			...
			for (uint32 X = 0; X < Width; X++)
			{
				float DeviceZ = *SrcPtr;
				float LinearValue = FMath::Min(InFlags.ComputeNormalizedDepth(DeviceZ), 1.0f);
				*DestPtr = FLinearColor(LinearValue, LinearValue, LinearValue, 0);
	...
	else
	{
		...
		for (uint32 Y = 0; Y < Height; Y++)
		{
			...
			for (uint32 X = 0; X < Width; X++)
			{
				// TODO
				uint8 DeviceStencil = *SrcPtr;
				*DestPtr = FLinearColor(DeviceStencil, DeviceStencil, DeviceStencil, 0xFF);
