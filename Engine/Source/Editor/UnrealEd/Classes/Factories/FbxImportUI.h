...
public:
	...
	/** If true, the imported static meshes will be relative to a tile of 2x2 km */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, config, Category = Mesh, meta = (ImportType = "StaticMesh", DisplayName = "Force vertex relative to tile (CARLA)"))
	uint32 bForceVerticesRelativeToTile:1;

	/** Tile size, by default and as maximum 2km per side*/
	UPROPERTY(EditAnywhere, BlueprintReadWrite, config, Category = Mesh, meta = (ImportType = "StaticMesh", DisplayName = "Tile size (CARLA)"))
	int32 TileSize;
