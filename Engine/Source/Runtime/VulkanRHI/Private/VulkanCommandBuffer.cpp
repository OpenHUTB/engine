...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FVulkanCommandBufferManager::SubmitUploadCmdBuffer);
	FScopeLock ScopeLock(&Pool.CS);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE("PrepareForNewActiveCommandBuffer");
	FScopeLock ScopeLock(&Pool.CS);
	check(!UploadCmdBuffer);
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FVulkanCommandBufferManager::GetUploadCmdBuffer);
	FScopeLock ScopeLock(&Pool.CS);
	if (!UploadCmdBuffer)
	{
		if (GVulkanUploadCmdBufferSemaphore)
