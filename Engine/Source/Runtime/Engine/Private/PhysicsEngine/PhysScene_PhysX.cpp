...
void FPhysScene_PhysX::InitPhysScene(const AWorldSettings* Settings)
{
	...
	/*
	// This is a debug output to check the phyx worker pool compiled in release
	ANSICHAR Msg[30];
	memset(Msg, 0, sizeof(Msg));
	sprintf(Msg, "PhysX workers from thread pool: %d\n", CPUDispatcher->getWorkerCount());
	write(STDOUT_FILENO, Msg, sizeof(Msg));
	*/
	UE_LOG(LogInit, Log, TEXT("PhysX workers from thread pool: %d"), CPUDispatcher->getWorkerCount());
