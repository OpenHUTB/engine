...
class FCARLACopySceneDepthPS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FCARLACopySceneDepthPS);
	SHADER_USE_PARAMETER_STRUCT(FCARLACopySceneDepthPS, FGlobalShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER(FVector2D, TextureExtent)
...
IMPLEMENT_GLOBAL_SHADER(FCARLACopySceneDepthPS, "/Engine/Private/CARLA/CopySceneDepth.usf", "Main", SF_Pixel);

class FCARLACopySceneStencilPS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FCARLACopySceneStencilPS);
	SHADER_USE_PARAMETER_STRUCT(FCARLACopySceneStencilPS, FGlobalShader);
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER(FVector2D, TextureExtent)
		SHADER_PARAMETER_STRUCT_REF(FViewUniformShaderParameters, View)
		SHADER_PARAMETER_RDG_TEXTURE_SRV(Texture2D<uint4>, SceneStencil)
		RENDER_TARGET_BINDING_SLOTS()
	END_SHADER_PARAMETER_STRUCT()
};

IMPLEMENT_GLOBAL_SHADER(FCARLACopySceneStencilPS, "/Engine/Private/CARLA/CopySceneStencil.usf", "Main", SF_Pixel);

class FCARLACopyCustomStencilPS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FCARLACopyCustomStencilPS);
	SHADER_USE_PARAMETER_STRUCT(FCARLACopyCustomStencilPS, FGlobalShader);
...
IMPLEMENT_GLOBAL_SHADER(FCARLACopyCustomStencilPS, "/Engine/Private/CARLA/CopyCustomStencil.usf", "Main", SF_Pixel);

class FCARLACopyVelocityPS : public FGlobalShader
{
	DECLARE_GLOBAL_SHADER(FCARLACopyVelocityPS);
	SHADER_USE_PARAMETER_STRUCT(FCARLACopyVelocityPS, FGlobalShader);
...
IMPLEMENT_GLOBAL_SHADER(FCARLACopyVelocityPS, "/Engine/Private/CARLA/CopyVelocity.usf", "Main", SF_Pixel);



static bool IsDummyGBufferTexture(FRDGTextureRef Texture)
{
	if (Texture == nullptr)
		return true;
	auto Extent = Texture->Desc.Extent;
	return Extent.X == 1 || Extent.Y == 1;
}

static bool IsDummyGBufferTexture(FRDGTextureSRVRef Texture)
{
	if (Texture == nullptr)
		return true;
	auto Extent = Texture->Desc.Texture->Desc.Extent;
	return Extent.X == 1 || Extent.Y == 1;
}

static bool IsDummyGBufferTexture(FShaderResourceViewRHIRef TextureSRV)
{
...
static void ExtractGBufferTexture(
	FRDGBuilder& GraphBuilder,
	FGBufferRequest* InOutGBufferRequest,
	FRDGTextureRef Texture,
	size_t i)
{
	auto Readback = InOutGBufferRequest->Readbacks[i].Get();
	check(Readback != nullptr);
	check(Texture->Desc.Format != PF_Unknown);
	AddReadbackTexturePass(
		GraphBuilder,
		RDG_EVENT_NAME("CARLA GBUFFER READBACK"),
		Texture,
		[Readback, Texture](FRHICommandList& RHICmdList)
		{
			Readback->EnqueueCopyRDG(RHICmdList, Texture->GetRHI());
		});
}

static void ExtractGBufferDepth(
	FRDGBuilder& GraphBuilder,
	FGBufferRequest* InOutGBufferRequest,
	FViewInfo* SourceView,
	FRDGTextureRef Texture,
	size_t i)
{
	if (IsDummyGBufferTexture(Texture))
	{
		(void)InOutGBufferRequest->DiscardedTextureMask.fetch_or(UINT64_C(1) << i, std::memory_order_release);
		return;
	}

	auto Extent = Texture->Desc.Extent;
	auto TargetTexture = GraphBuilder.CreateTexture(
		FRDGTextureDesc::Create2D(
			Extent, PF_FloatRGBA,
			FClearValueBinding::Black, TexCreate_RenderTargetable),
		TEXT("SCENE DEPTH COPY"),
		ERDGTextureFlags::None);

	TShaderMapRef<FCARLACopySceneDepthPS> PixelShader(SourceView->ShaderMap);
	auto Parameters = GraphBuilder.AllocParameters<FCARLACopySceneDepthPS::FParameters>();
	Parameters->SceneDepth = Texture;
	...
	Parameters->View = SourceView->ViewUniformBuffer;
	Parameters->RenderTargets[0] = FRenderTargetBinding(TargetTexture, ERenderTargetLoadAction::EClear);

	FPixelShaderUtils::AddFullscreenPass(GraphBuilder,
		SourceView->ShaderMap,
		RDG_EVENT_NAME("CARLA GBUFFERS: COPY SCENE/CUSTOM DEPTH"),
		PixelShader,
		Parameters,
		InOutGBufferRequest->ViewRect);

	ExtractGBufferTexture(GraphBuilder, InOutGBufferRequest, TargetTexture, i);
}

static void ExtractGBufferVelocity(
	FRDGBuilder& GraphBuilder,
	FGBufferRequest* InOutGBufferRequest,
	FViewInfo* SourceView,
	FRDGTextureRef Texture,
	size_t i)
{
	...
	TShaderMapRef<FCARLACopyVelocityPS> PixelShader(SourceView->ShaderMap);
	auto Parameters = GraphBuilder.AllocParameters<FCARLACopyVelocityPS::FParameters>();
	Parameters->SceneVelocity = Texture;
	Parameters->SceneVelocitySampler = TStaticSamplerState<SF_Point, AM_Clamp, AM_Clamp, AM_Clamp>::GetRHI();
	Parameters->TextureExtent = Extent;
	Parameters->View = SourceView->ViewUniformBuffer;
	Parameters->RenderTargets[0] = FRenderTargetBinding(TargetTexture, ERenderTargetLoadAction::EClear);
	...
	FPixelShaderUtils::AddFullscreenPass(GraphBuilder,
		SourceView->ShaderMap,
		RDG_EVENT_NAME("CARLA GBUFFERS: COPY VELOCITY"),
		PixelShader,
		Parameters,
		InOutGBufferRequest->ViewRect);
...
static void ExtractGBufferStencil(
	FRDGBuilder& GraphBuilder,
	FGBufferRequest* InOutGBufferRequest,
	FViewInfo* SourceView,
	FRDGTextureSRVRef StencilTexture,
	size_t i)
{
	if (IsDummyGBufferTexture(StencilTexture))
	{
		(void)InOutGBufferRequest->DiscardedTextureMask.fetch_or(UINT64_C(1) << i, std::memory_order_release);
		return;
	}

	auto Extent = StencilTexture->Desc.Texture->Desc.Extent;
	auto TargetTexture = GraphBuilder.CreateTexture(
		FRDGTextureDesc::Create2D(
			Extent, PF_R8G8B8A8,
			FClearValueBinding::Black, TexCreate_RenderTargetable),
		TEXT("SCENE STENCIL COPY"),
		ERDGTextureFlags::None);

	TShaderMapRef<FCARLACopySceneStencilPS> PixelShader(SourceView->ShaderMap);
	auto Parameters = GraphBuilder.AllocParameters<FCARLACopySceneStencilPS::FParameters>();
	Parameters->SceneStencil = StencilTexture;
	Parameters->TextureExtent = Extent;
	Parameters->View = SourceView->ViewUniformBuffer;
	Parameters->RenderTargets[0] = FRenderTargetBinding(TargetTexture, ERenderTargetLoadAction::EClear);
	...
	FPixelShaderUtils::AddFullscreenPass(GraphBuilder,
		SourceView->ShaderMap,
		RDG_EVENT_NAME("CARLA GBUFFERS: COPY SCENE/CUSTOM STENCIL"),
		PixelShader,
		Parameters,
		InOutGBufferRequest->ViewRect);
	...
	size_t i)
{
	...
	ExtractGBufferTexture(GraphBuilder, InOutGBufferRequest, Texture, i);
};
...
{
	Render_CARLA(RHICmdList, nullptr);
}

void FDeferredShadingSceneRenderer::Render_CARLA(FRHICommandListImmediate& RHICmdList, FGBufferRequest* InOutGBufferRequest)
{
	...
	FViewInfo* SourceViewForGBufferCapture = nullptr;

	if (InOutGBufferRequest != nullptr)
		SourceViewForGBufferCapture = InOutGBufferRequest->InitAndFindSceneView(Views);

	if (InOutGBufferRequest != nullptr && InOutGBufferRequest->IsRequested(EGBufferTextureID::SceneDepth))
	{
		ExtractGBufferDepth(
			GraphBuilder,
			InOutGBufferRequest,
			SourceViewForGBufferCapture,
			SceneDepthTexture.Target,
			(size_t)EGBufferTextureID::SceneDepth);
	}

	if (InOutGBufferRequest != nullptr && InOutGBufferRequest->IsRequested(EGBufferTextureID::SceneStencil))
	{
		ExtractGBufferStencil(
			GraphBuilder,
			InOutGBufferRequest,
			SourceViewForGBufferCapture,
			SceneStencilTexture,
			(size_t)EGBufferTextureID::SceneStencil);
	}
	...
	if (InOutGBufferRequest != nullptr && InOutGBufferRequest->IsRequested(EGBufferTextureID::CustomStencil))
	{
		ExtractGBufferStencil(
			GraphBuilder,
			InOutGBufferRequest,
			SourceViewForGBufferCapture,
			GraphBuilder.CreateSRV(FRDGTextureSRVDesc::CreateWithPixelFormat((*SceneTextures)->CustomDepthTexture, PF_X24_G8)),
			(size_t)EGBufferTextureID::CustomStencil);
	}

	if (InOutGBufferRequest != nullptr && InOutGBufferRequest->IsRequested(EGBufferTextureID::CustomDepth))
	{
		ExtractGBufferDepth(
			GraphBuilder,
			InOutGBufferRequest,
			SourceViewForGBufferCapture,
			(*SceneTextures)->CustomDepthTexture,
			(size_t)EGBufferTextureID::CustomDepth);
	}
	...
	if (InOutGBufferRequest != nullptr && InOutGBufferRequest->IsRequested(EGBufferTextureID::Velocity))
	{
		ExtractGBufferVelocity(
			GraphBuilder,
			InOutGBufferRequest,
			SourceViewForGBufferCapture,
			(*SceneTextures)->GBufferVelocityTexture,
			(size_t)EGBufferTextureID::Velocity);
	}
	...
	if (InOutGBufferRequest != nullptr)
	{
		check(SourceViewForGBufferCapture != nullptr);

		FRDGTextureRef GBufferTextures[] =
		{
			nullptr,
			(*SceneTextures)->SceneDepthTexture, /* SceneStencil */ nullptr,
			(*SceneTextures)->GBufferATexture, (*SceneTextures)->GBufferBTexture,
			(*SceneTextures)->GBufferCTexture, (*SceneTextures)->GBufferDTexture,
			(*SceneTextures)->GBufferETexture, (*SceneTextures)->GBufferFTexture,
			nullptr, (*SceneTextures)->ScreenSpaceAOTexture
		};

		auto ShouldHandleSeparately = [](size_t Index)
		{
			constexpr uint8 ExcludedIndices[] =
			{
				(uint8)EGBufferTextureID::SceneDepth,
				(uint8)EGBufferTextureID::SceneStencil,
				(uint8)EGBufferTextureID::CustomDepth,
				(uint8)EGBufferTextureID::CustomStencil,
				(uint8)EGBufferTextureID::Velocity
			};

			for (auto e : ExcludedIndices)
				if (e == Index)
					return true;
			return false;
		};

		// Start at index 1, we'll handle the color later
		for (size_t i = 1; i != FGBufferRequest::TextureCount; ++i)
		{
			if (ShouldHandleSeparately(i) || !InOutGBufferRequest->IsRequested((EGBufferTextureID)i))
				continue;
