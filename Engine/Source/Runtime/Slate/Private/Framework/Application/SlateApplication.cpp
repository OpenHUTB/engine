...
int32 GRenderWindowEvenWindowMinimized = 0;
static FAutoConsoleVariableRef CVarActorClusteringEnabled(
	TEXT("r.Carla.RenderWindowEvenWindowMinimized"),
	GRenderWindowEvenWindowMinimized,
	TEXT("Whether to allow Render Window When Window Minimized(Bug Fix: Render Resource Not Release When Window Minimize)."),
	ECVF_Default
);
...
void FSlateApplication::DrawWindowAndChildren( const TSharedRef<SWindow>& WindowToDraw, FDrawWindowArgs& DrawWindowArgs )
{
	...
	if (bRenderOffScreen || (WindowToDraw->IsVisible() && (GRenderWindowEvenWindowMinimized || !WindowToDraw->IsWindowMinimized() || FApp::UseVRFocus())) )
	{
