...
void UWheeledVehicleMovementComponent::SetupWheelMassProperties_AssumesLocked(const uint32 NumWheels, PxVehicleWheelsSimData* PWheelsSimData, PxRigidBody* PVehicleActor)
{
	...
	for (uint32 WheelIdx = 0; WheelIdx < NumWheels; WheelIdx++)
	{
		...
		PSuspensionData.mCamberAtRest = Wheel->CamberAngleAtRest;
		PSuspensionData.mCamberAtMaxDroop = Wheel->CamberAngleAtMaxDroop;
		PSuspensionData.mCamberAtMaxCompression = Wheel->CamberAngleAtMaxCompression;

		const FVector PSuspensionTravelDirections = Wheel->SuspensionTravelDirection;
		...
		PWheelsSimData->setSuspTravelDirection(WheelIdx, PxVec3(PSuspensionTravelDirections.X, PSuspensionTravelDirections.Y, PSuspensionTravelDirections.Z));
	...
	uint32 numAntiRollBars = AntiRollBarSetups.Num();
	if (numAntiRollBars > 0 && numAntiRollBars <= (NumWheels / 2))
	{
		for (uint32 AntiRollBarIdx = 0; AntiRollBarIdx < numAntiRollBars; AntiRollBarIdx++)
		{
			physx::PxVehicleAntiRollBarData mtData;
			mtData.mWheel0 = AntiRollBarSetups[AntiRollBarIdx].Wheel0;
			mtData.mWheel1 = AntiRollBarSetups[AntiRollBarIdx].Wheel1;
			mtData.mStiffness = AntiRollBarSetups[AntiRollBarIdx].Stiffness;

			PWheelsSimData->addAntiRollBarData(mtData);
		}
	}
}
#endif
...
void UWheeledVehicleMovementComponent::DrawDebug(UCanvas* Canvas, float& YL, float& YPos)
{
	...
	for (uint32 w = 0; w < PVehicle->mWheelsSimData.getNbWheels(); ++w)
	{
		...
		Canvas->DrawText(RenderFont, FString::Printf(TEXT("Tire Friction: %.3f"), WheelRPM), GetXPos(120.f), YPos);
		Canvas->DrawText(RenderFont, FString::Printf(TEXT("Slip Ratio: %.2f"), TireFriction), GetXPos(160.f), YPos);
		Canvas->DrawText(RenderFont, FString::Printf(TEXT("Slip Angle (degrees): %.1f"), FMath::RadiansToDegrees(LatSlip)), GetXPos(200.f), YPos);
		Canvas->DrawText(RenderFont, FString::Printf(TEXT("Contact Surface: %s"), *ContactSurfaceString), GetXPos(280.f), YPos);
...
void UWheeledVehicleMovementComponent::StopTelemetry()
{
	if (PVehicle == NULL)
	{
		return;
	...
	FPhysXVehicleManager* MyVehicleManager = FPhysXVehicleManager::GetVehicleManagerFromScene(GetWorld()->GetPhysicsScene());
	MyVehicleManager->SetRecordTelemetry(this, false);
}
...
FAntiRollBarSetup::FAntiRollBarSetup()
: Wheel0(0)
, Wheel1(0)
, Stiffness(0.0f)
{
