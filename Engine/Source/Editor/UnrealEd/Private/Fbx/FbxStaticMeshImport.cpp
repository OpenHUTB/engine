...
bool UnFbx::FFbxImporter::BuildStaticMeshFromGeometry(FbxNode* Node, UStaticMesh* StaticMesh, TArray<FFbxMaterial>& MeshMaterials, int32 LODIndex,
	...
	bool bConvertToRelativeTile, int32 TileCenterx, int32 TileCentery)
{
	...
	bool bHasNonDegeneratePolygons = false;
	{
		...
		for (int32 VertexIndex = 0; VertexIndex < VertexCount; ++VertexIndex)
		{
			...
			// relocate the vertex to its own tile
			if (bConvertToRelativeTile)
			{
				// relocate vertex
				FbxPosition[0] -= TileCenterx;
				FbxPosition[1] -= TileCentery;
			}
	...
	for (int32 MeshIndex = 0; MeshIndex < MeshNodeArray.Num(); MeshIndex++)
	{
		...
		if (Node->GetMesh())
		{
			...
			if (SqrSize > SqrBoundingBoxThreshold)
			{
				...
				int32 TileCenterx = 0, TileCentery = 0;

				FbxVector4 GlobalLoc = Node->EvaluateGlobalTransform(0).GetT();

				if (TemplateImportData->bForceVerticesRelativeToTile)
				{
					int32 TileSize = TemplateImportData->TileSize;
					// check limit imposed by Unreal Engine
					if (TileSize < 1) 	 TileSize = 1;
					if (TileSize > 2000) TileSize = 2000;
					// get position of the center of the bounding box in meters, without decimals
					int32 Posx = static_cast<int32>((BBoxMin[0] + ((BBoxMax[0] - BBoxMin[0]) / 2))/100);
					int32 Posy = static_cast<int32>((BBoxMin[1] + ((BBoxMax[1] - BBoxMin[1]) / 2))/100);
					int32 Tilei = Posx / TileSize;
					int32 Tilej = Posy / TileSize;
					double TileHalf = TileSize * 0.5;
					if (Posx > 0)
						TileCenterx = (Tilei * TileSize) + TileHalf;
					else
						TileCenterx = (Tilei * TileSize) - TileHalf;
					if (Posy > 0)
						TileCentery = (Tilej * TileSize) + TileHalf;
					else
						TileCentery = (Tilej * TileSize) - TileHalf;
						
					// write the center of the tile (0,0)
					if (MeshName.Contains("_Tile_0_0"))
					{
						FString Folder = FPackageName::GetLongPackagePath(InParent->GetOutermost()->GetName());
						Folder = Folder.Replace(TEXT("/Game/"), TEXT("Content/"), ESearchCase::IgnoreCase);
						FString GameDir = FPaths::ConvertRelativePathToFull(FPaths::ProjectDir());
						FString FullPath = GameDir + Folder + TEXT("/TilesInfo.txt");
						WriteGridTileStartInfo(FullPath, TileCenterx, -TileCentery, TileSize);
					}
				}
				...
				if (!BuildStaticMeshFromGeometry(Node, StaticMesh, MeshMaterials, LODIndex,
					...
					TemplateImportData->bForceVerticesRelativeToTile, TileCenterx, TileCentery))
				{
					bBuildStatus = false;
					break;
				}
...
void UnFbx::FFbxImporter::WriteGridTileStartInfo(FString Filename, int32 Centerx, int32 Centery, int32 TileSize)
{
	IPlatformFile& FileManager = FPlatformFileManager::Get().GetPlatformFile();
	FileManager.CreateDirectoryTree(*FPaths::GetPath(Filename));

	std::ofstream File;
	File.open(TCHAR_TO_UTF8(*Filename));
	File << Centerx << "," << Centery << "," << TileSize << "\n";
	File.close();	
}
